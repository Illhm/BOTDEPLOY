...
class ProcessManager:
    ...
    async def setup_dependencies(self, script_path: Path, requirements_file: Optional[Path] = None) -> tuple[Optional[Path], str]:
        ...
        # Create venv directory (use absolute path to avoid cwd issues)
        venv_dir = Path(getattr(config, 'VENV_DIR', './venvs')).resolve()
        timestamp = datetime.now().timestamp()
        venv_path = (venv_dir / f"venv_{timestamp}").resolve()
    ...
    @staticmethod
    def _resolve_python_executable(venv_path: Optional[Path]) -> Path:
        """Return the best Python executable for the provided venv or system."""
    ...
    def run_script(
        ...
        try:
            python_path = self._resolve_python_executable(venv_path)

            if venv_path and not python_path.exists():
    ...
    async def _restart_process(self, old_process: ProcessInfo):
        ...
        try:
            ...
            python_path = self._resolve_python_executable(old_process.venv_path)

            if old_process.venv_path and not python_path.exists():
                logger.error(
                    "Python executable missing for venv %s; aborting restart", old_process.venv_path
                )
